...
$url = new moodle_url('/admin/tool/mfa/factor/telegram/telegram.php',
    array('instance' => $instanceid, 'pass' => $pass, 'secret' => $secret));
$PAGE->set_url($url);
$PAGE->set_pagelayout('secure');
$PAGE->set_title(get_string('unauthtelegram', 'factor_telegram'));
...
$form = new \factor_telegram\form\telegram($url);
...
if ($form->is_cancelled()) {
    ...
    if (empty($instance)) {
        $message = get_string('error:badcode', 'factor_telegram');
    } else {
        ...
        $DB->set_field('tool_mfa', 'revoked', 1, array('userid' => $user->id, 'factor' => 'telegram'));
        ...
        $event = \factor_telegram\event\unauth_telegram::unauth_telegram_event($user, $ip, $useragent);
        $event->trigger();

        // Suspend user account.
        if (get_config('factor_telegram', 'suspend')) {
            $DB->set_field('user', 'suspended', 1, array('id' => $userid));
        }

        $message = get_string('telegram:revokesuccess', 'factor_telegram', fullname($user));
    }
}

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('unauthtelegram', 'factor_telegram'));
if (!empty($message)) {
    echo $message;
} else {
    $form->display();
